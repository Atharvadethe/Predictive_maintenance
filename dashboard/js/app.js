/* ===================================================================
   ...
   Carrier template layout: navy topbar + compact sidebar
   =================================================================== */
...
const APP = (() => {
    ...
    function renderTopbar() {
        ...
        const navItems = [
            { id: 'index',       icon: 'fa-th-large',            label: 'Overview',           href: 'index.html' },
            { id: 'asset-health',icon: 'fa-heartbeat',           label: 'Asset Health',       href: 'asset-health.html' },
            { id: 'anomalies',   icon: 'fa-exclamation-triangle',label: 'Anomalies',          href: 'anomalies.html',
              badge: DATA.alerts.filter(a => !a.acknowledged && a.level === 'Critical').length },
            ...
            { id: 'maintenance', icon: 'fa-wrench',              label: 'Maintenance',        href: 'maintenance.html',
              badge: DATA.workOrders.filter(w => w.status === 'Open').length },
            { id: 'trends',      icon: 'fa-chart-line',          label: 'Trends',             href: 'trends.html' },
        ];
        ...
        header.innerHTML = `
            ...
            <div class="navbar">
                ...
                <div class="navbar-right">
                    ...
                    <div class="search-box" id="searchBox" style="margin-right: 4px;">
    ...
    // ─── Shell: sidebar + wrap main-content ───
    function renderShell() {
        ...
        // Add footer inside main-content
        renderFooter();
    }
    ...
    function renderNotificationPanel() {
        ...
        panel.innerHTML = `
            <div class="notification-panel-header">
                <h3>Notifications</h3>
                <button class="btn-secondary" id="markAllRead" style="padding:4px 10px;font-size:10.5px">Mark all read</button>
            </div>
    ...
    function getHealthColor(hi) {
        if (hi >= 80) return themeVar('--green', '#00875A');
        if (hi >= 55) return themeVar('--amber', '#B45309');
        return themeVar('--red', '#D92D20');
    }

    function getHealthGradient(hi) {
        const c = getHealthColor(hi);
        ...
        return `
            <div class="progress-ring" style="width:${size}px;height:${size}px">
                <svg width="${size}" height="${size}">
                    <circle cx="${size/2}" cy="${size/2}" r="${radius}" fill="none" stroke="var(--canvas)" stroke-width="${strokeWidth}"/>
                ...
                <span class="progress-ring-text" style="font-size:${size > 50 ? 13 : 11}px">${Math.round(value)}</span>
            </div>
        `;
    }
